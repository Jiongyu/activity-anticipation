...
"""
activity = '0510142336'  #you could choose another activity
top_activity = 'having_meal'
fold = 'fold_2' # activity locating in the fold
person = 'person2' # cad120 dataset person fold
"""

activity = '1204180344'  #you could choose another activity
top_activity = 'microwaving_food'
fold = 'fold_4' # activity locating in the fold
person = 'person2' # cad120 dataset person fold


def main():

    rospy.init_node("{1}_{0}".format(activity,top_activity), anonymous=True)
    image_pub = rospy.Publisher("{1}_{0}".format(activity,top_activity),Image,queue_size=1)
    bridge = CvBridge()

    while(image_pub.get_num_connections() < 1):
        print "the number of connections of the {1}_{0} still less than 1.".format(activity,top_activity)
        time.sleep(1)

    #CAD-120 dataset
    dataset_CAD120_path = './src/activity-anticipation/src/CAD_120'
    # having_meal which we can change to another is topper activity 
    image_path = "{0}/{3}/Subject3_rgbd_images/{2}/{1}/".format(dataset_CAD120_path , activity,top_activity,person) 
    ground_truth_label_activity = '{0}/{2}/Subject3_annotations/{1}/labeling.txt'.format(dataset_CAD120_path,top_activity,person)
    object1_location =  '{0}/{3}/Subject3_annotations/{2}/{1}_obj1.txt'.format(dataset_CAD120_path,activity,top_activity,person)
    object2_location =  '{0}/{3}/Subject3_annotations/{2}/{1}_obj2.txt'.format(dataset_CAD120_path,activity,top_activity,person)
    ...
    ground_truth_path = './src/activity-anticipation/src/activity-anticipation/dataset/{0}/'.format(fold)
    ...
    prediction_human_and_object_path = './src/activity-anticipation/src/activity-anticipation/prediction/{0}/'.format(fold)
    detection_human_data = cPickle.load(open('{0}prediction_detection_human_579188.pik'.format(prediction_human_and_object_path)))
    anticipation_human_data = cPickle.load(open('{0}prediction_anticipation_human_579188.pik'.format(prediction_human_and_object_path)))
    detection_object_data = cPickle.load(open('{0}prediction_detection_object_579188.pik'.format(prediction_human_and_object_path)))
    anticipation_object_data = cPickle.load(open('{0}prediction_anticipation_object_579188.pik'.format(prediction_human_and_object_path)))
    ...
    truth_human = ground_truth_label_human[1].T[0]
    detect_human = detection_human_data[1].T[0]
    anticipation_human = anticipation_human_data[1].T[0]
    truth_object = ground_truth_label_object[1].T[0]
    detect_object = detection_object_data[1].T[0]
    anticipation_object = anticipation_object_data[1].T[0]
    ...
    while (segement < len(start_frame)):
        ...
        while((not rospy.is_shutdown()) and index <= end_frame[segement]):
            ...
            cv2.putText(data,'ground truth human label:{0}'.format(dic_ground_truth_label_human[truth_human[segement]]),(30,50), font, 0.5,(0,0,0),1,cv2.LINE_AA)
            cv2.putText(data,'ground truth object label:{0}'.format(dic_ground_truth_label_object[truth_object[segement]]),(30,70), font, 0.5,(0,0,0),1,cv2.LINE_AA)
            ...
            cv2.putText(data,"cub",(point_object1_location[point],point_object1_location[point+1]-10), font, 0.75,(0,255,0),1,cv2.LINE_AA)
            ...
            cv2.putText(data,"apple",(point_object2_location[point],point_object2_location[point+1]-10), font, 0.75,(0,255,0),1,cv2.LINE_AA)


            #print '{0}RGB_{1}.png'.format(image_path,index)
            #cv2.putText(data,'OpenCV',(10,500), font, 4,(255,255,255),2,cv2.LINE_AA)
            cv2.putText(data,'detect human label:{0}'.format(dic_ground_truth_label_human[detect_human[segement]]),(30,90), font, 0.5,(0,0,0),1,cv2.LINE_AA)
            cv2.putText(data,'detect object label:{0}'.format(dic_ground_truth_label_object[detect_object[segement]]),(30,110), font, 0.5,(0,0,0),1,cv2.LINE_AA)
            cv2.putText(data,'anticipation human label:{0}'.format(dic_ground_truth_label_human[anticipation_human[segement]]),(30,130), font, 0.5,(0,0,0),1,cv2.LINE_AA)
            cv2.putText(data,'anticipation object label:{0}'.format(dic_ground_truth_label_object[anticipation_object[segement]]),(30,150), font, 0.5,(0,0,0),1,cv2.LINE_AA)
