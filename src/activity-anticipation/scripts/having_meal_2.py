...
def main():
    ...
    rospy.init_node("having_meal_{0}".format(activity), anonymous=True)
    image_pub = rospy.Publisher("having_meal_{0}".format(activity),Image,queue_size=1)
    bridge = CvBridge()

    while(image_pub.get_num_connections() < 1):
        print "the number of connections of the having_meal_{0} still less than 1.".format(activity)
        time.sleep(1)

    #CAD-120 dataset
    dataset_CAD120_path = './src/activity-anticipation/src/CAD_120'
    # having_meal which we can change to another is topper activity 
    image_path = "{0}/person2/Subject3_rgbd_images/having_meal/{1}/".format(dataset_CAD120_path , activity) 
    ground_truth_label_activity = '{0}/person2/Subject3_annotations/having_meal/labeling.txt'.format(dataset_CAD120_path)
    object1_location =  '{0}/person2/Subject3_annotations/having_meal/{1}_obj1.txt'.format(dataset_CAD120_path,activity)
    object2_location =  '{0}/person2/Subject3_annotations/having_meal/{1}_obj2.txt'.format(dataset_CAD120_path,activity)
