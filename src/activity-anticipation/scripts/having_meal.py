...
activity = '0510182057'  #you could choose another activity
...
def main():
    ...
    image_path = "{0}/person1/Subject1_rgbd_images/having_meal/{1}/".format(dataset_CAD120_path , activity) 
    ground_truth_label_activity = '{0}/person1/Subject1_annotations/having_meal/labeling.txt'.format(dataset_CAD120_path)
    object_location =  '{0}/person1/Subject1_annotations/having_meal/{1}_obj1.txt'.format(dataset_CAD120_path,activity)
    ...
    ground_truth_path = './src/activity-anticipation/src/activity-anticipation/dataset/fold_3/'
    ...
    prediction_human_and_object_path = './src/activity-anticipation/src/activity-anticipation/prediction/fold_3/'
    ...
    #the number 8 is mean that the 9 line(a activity) of the fold3 file in the activity fold.
    #so following above change,you should to change the number. 
    truth_human = ground_truth_label_human[8].T[0]
    detect_human = detection_human_data[8].T[0]
    anticipation_human = anticipation_human_data[8].T[0]
    truth_object = ground_truth_label_object[8].T[0]
    detect_object = detection_object_data[8].T[0]
    anticipation_object = anticipation_object_data[8].T[0]
    ...
    while (segement < len(start_frame)):
        ...
        while((not rospy.is_shutdown()) and index <= end_frame[segement]):
            ...
            cv2.putText(data,"cub",(point_object_location[point],point_object_location[point+1]-10), font, 0.75,(0,255,0),1,cv2.LINE_AA)
