...
start_frame =[]
end_frame = []
activity = '0510144139'  #you could choose another activity

def main():

    rospy.init_node("taking_food_{0}".format(activity), anonymous=True)
    image_pub = rospy.Publisher("taking_food_{0}".format(activity),Image,queue_size=1)
    bridge = CvBridge()

    while(image_pub.get_num_connections() < 1):
        print "the number of connections of the taking_food_{0} still less than 1.".format(activity)
        time.sleep(1)

    #CAD-120 dataset
    dataset_CAD120_path = './src/activity-anticipation/src/CAD_120'
    # taking_food which we can change to another is topper activity 
    image_path = "{0}/person2/Subject3_rgbd_images/taking_food/{1}/".format(dataset_CAD120_path , activity) 
    ground_truth_label_activity = '{0}/person2/Subject3_annotations/taking_food/labeling.txt'.format(dataset_CAD120_path)
    object1_location =  '{0}/person2/Subject3_annotations/taking_food/{1}_obj1.txt'.format(dataset_CAD120_path,activity)
    object2_location =  '{0}/person2/Subject3_annotations/taking_food/{1}_obj2.txt'.format(dataset_CAD120_path,activity)
    ...
    ground_truth_path = './src/activity-anticipation/src/activity-anticipation/dataset/fold_2/'
    ...
    prediction_human_and_object_path = './src/activity-anticipation/src/activity-anticipation/prediction/fold_2/'
    ...
    truth_human = ground_truth_label_human[7].T[0]
    detect_human = detection_human_data[7].T[0]
    anticipation_human = anticipation_human_data[7].T[0]
    truth_object = ground_truth_label_object[7].T[0]
    detect_object = detection_object_data[7].T[0]
    anticipation_object = anticipation_object_data[7].T[0]
    ...
    while (segement < len(start_frame)):
        ...
        while((not rospy.is_shutdown()) and index <= end_frame[segement]):
            ...
            cv2.putText(data,"microwaving",(point_object1_location[point],point_object1_location[point+1]-10), font, 0.75,(0,255,0),1,cv2.LINE_AA)
            ...
            cv2.putText(data,"bowl",(point_object2_location[point],point_object2_location[point+1]-10), font, 0.75,(0,255,0),1,cv2.LINE_AA)
