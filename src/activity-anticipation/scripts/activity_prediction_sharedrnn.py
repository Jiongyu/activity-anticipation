...
def predict_activitiy(index,fold,checkpoint,architecture='joint'):
	...
	if architecture == 'detection':
		for xte_shared,xte,yte in zip(X_te_human_shared,X_te_human_disjoint,Y_te_human):
			...
			errors += len(t[0])
			N += yte.shape[0]
		cPickle.dump(human_label,open('{1}/prediction_{2}_human_{0}.pik'.format(index, path_to_prediction, architecture), 'wb'))
		del human_label
		...
		cPickle.dump(object_label,open('{1}/prediction_{2}_object_{0}.pik'.format(index, path_to_prediction, architecture), 'wb'))
		del object_label
